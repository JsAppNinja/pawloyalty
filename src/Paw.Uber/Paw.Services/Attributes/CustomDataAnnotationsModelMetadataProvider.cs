...
namespace Paw.Services.Attributes
{
    public class CustomDataAnnotationsModelMetadataProvider : DataAnnotationsModelMetadataProvider 
    {
                                                    ...
                                                    string propertyName)
        {
            ...
            if (propertyInfo != null)
            {
                List<IExecuteValue> executeValueList = AttributeHelper.GetPropertyAttributeList<IExecuteValue>(propertyInfo);
                if (executeValueList.Count > 0)
                {
                    metadata.AdditionalValues.Add("ExecuteValueList", executeValueList);
                }
            }
