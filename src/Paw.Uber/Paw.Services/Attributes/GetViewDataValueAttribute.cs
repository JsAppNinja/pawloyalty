...
namespace Paw.Services.Attributes
{
    public class GetViewDataValueAttribute : Attribute, IBindControllerValue
    {
        ...
        public virtual object GetValue(object model, ControllerBase controller)
        {
            return controller.ViewData[this.ParameterName];
        }

        public virtual void BindControllerValue(object model, ControllerBase controller, object item)
        {
            this.PropertyInfo.SetValue(item, this.GetValue(model, controller));
        }
    }
}
