...
namespace Paw.Services.Common
{
    public class Pet : IId, IProviderGroupId
    {
        ...
        public ICollection<PetNote> PetNoteCollection { get; set; }

        public Guid? ColorMarkingId
        {
            get { return _ColorMarkingId; }
            set { _ColorMarkingId = value; }
        }
        private Guid? _ColorMarkingId = null;

        [ForeignKey("ColorMarkingId")]
        public virtual ColorMarking ColorMarking { get; set; }

        #region display ...

        public string NameBreedAge
        {
            get
            {
                List<string> result = new List<string>();
                result.Add(this.Name);
                result.Add(this.Breed?.Name);
                result.Add(GetDOB(this.DOB));
                result.Add(string.Format("{0:0.0}", this.Weight));
